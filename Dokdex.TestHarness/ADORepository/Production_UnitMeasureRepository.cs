...
{
	public partial class Production_UnitMeasureRepository
	{        
		public void Export_Production_UnitMeasure()
		{
            DokdexClient client = new DokdexClient("http://localhost:6858/");

            if(client.Schema.Exists("AdventureWorks2012:Production:UnitMeasure"))
			{
				return;
			}
            ...
            client.Schema.Create("AdventureWorks2012:Production:UnitMeasure");
			...
			using (SqlConnection connection = new SqlConnection("Server=.;Database=AdventureWorks2012;Trusted_Connection=True;"))
			{
				...
				try
				{
					using (SqlCommand command = new SqlCommand("SELECT * FROM Production.UnitMeasure", connection))
					{
						...
						using (SqlDataReader dataReader = command.ExecuteReader(System.Data.CommandBehavior.CloseConnection))
						{
							...
							while (dataReader.Read() /*&& rowCount++ < 10000*/)
							{
								...
								{
									Console.WriteLine("AdventureWorks2012:Production:UnitMeasure: {0}", rowCount);
								}
