...
{
    public static class StaticParserDropIndex<TData> where TData : IStringable
    {
        internal static PreparedQuery<TData> Parse(QueryBatch<TData> queryBatch, Tokenizer<TData> tokenizer)
        {
            ...
            {
                throw new KbParserException(tokenizer.GetCurrentLineNumber(), "Invalid query. Found [{schemaName}], expected: schema name.");
            }
