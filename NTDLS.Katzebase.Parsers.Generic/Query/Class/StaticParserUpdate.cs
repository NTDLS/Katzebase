...
{
    public static class StaticParserUpdate<TData> where TData : IStringable
    {
        internal static PreparedQuery<TData> Parse(QueryBatch<TData> queryBatch, Tokenizer<TData> tokenizer, Func<string, TData> parseStringToDoc = null, Func<string, TData> castStringToDoc = null)
        {
            /*Example query:
             * update
	         *       Test
            ...
            while (!tokenizer.IsExhausted())
            {
                ...
                var queryField = StaticParserField<TData>.Parse(tokenizer, fieldExpression, query.UpdateFieldValues, parseStringToDoc, castStringToDoc);
