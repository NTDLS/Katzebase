...
namespace NTDLS.Katzebase.Parsers.Query.Class
{
    ...
    public class StaticParserField<TData> where TData : IStringable
    {
        /// <summary>
        /// Parses a field expression containing fields, functions. string and math operations.
        /// </summary>
        public static IQueryField<TData> Parse(Tokenizer<TData> parentTokenizer, string givenFieldText, QueryFieldCollection<TData> queryFields, Func<string, TData> parseStringToDoc, Func<string, TData> castStringToDoc)
        {
            Tokenizer<TData> tokenizer = new(givenFieldText, parseStringToDoc);
            ...
            if (tokenizer.IsExhausted()) //This is a single value (document field, number or string), the simple case.
            {
                if (token.IsQueryFieldIdentifier())
                {
                    if (ScalerFunctionCollection<TData>.TryGetFunction(token, out var _))
                    {
                        ...
                        throw new KbParserException(tokenizer.GetCurrentLineNumber(), $"Function scaler expects parentheses: [{token}]");
                    }
                    ...
                    var queryFieldDocumentIdentifier = new QueryFieldDocumentIdentifier<TData>(token, parseStringToDoc);
                ...
                else if (IsNumericExpression(token, parseStringToDoc))
                {
                    return new QueryFieldConstantNumeric<TData>(castStringToDoc(token));
                }
                else
                {
                    return new QueryFieldConstantString<TData>(castStringToDoc(token));
                }
            }
            ...
            if (validateNumberOfParameters.Count > 1)
            {
                ...
                throw new KbParserException(tokenizer.GetCurrentLineNumber(), $"Single field should not contain multiple values: [{givenFieldText}]");
            }

            //This field is going to require evaluation, so figure out if its a number or a string.
            if (IsNumericExpression(givenFieldText, parseStringToDoc))
            {
                IQueryFieldExpression<TData> expression = new QueryFieldExpressionNumeric<TData>(castStringToDoc(givenFieldText));
                expression.Value = castStringToDoc(ParseEvaluationRecursive(parentTokenizer, ref expression, givenFieldText, ref queryFields, parseStringToDoc));
                return expression;
            }
            else
            {
        ...
        private static string ParseEvaluationRecursive(Tokenizer<TData> parentTokenizer, ref IQueryFieldExpression<TData> rootQueryFieldExpression,
            string givenExpressionText, ref QueryFieldCollection<TData> queryFields, Func<string, TData> parseStringToDoc)
        {
            Tokenizer<TData> tokenizer = new(givenExpressionText, parseStringToDoc);
            ...
            while (!tokenizer.IsExhausted())
            {
                ...
                if (token == "(")
                {
                    ...
                    var parenthesesResult = ParseEvaluationRecursive(parentTokenizer, ref rootQueryFieldExpression, scopeText, ref queryFields, parseStringToDoc);
                ...
                else if (ScalerFunctionCollection<TData>.TryGetFunction(token, out var scalerFunction))
                {
                    ...
                    ParseFunctionCallRecursive(parentTokenizer, ref rootQueryFieldExpression, queryFieldExpressionFunction, ref queryFields, tokenizer, positionBeforeToken, parseStringToDoc);
                ...
                else if (token.IsQueryFieldIdentifier())
                {
                    ...
                    var fieldKey = queryFields.GetNextDocumentFieldKey();
                    queryFields.DocumentIdentifiers.Add(fieldKey, new QueryFieldDocumentIdentifier<TData>(token, parseStringToDoc));
        ...
        private static void ParseFunctionCallRecursive(Tokenizer<TData> parentTokenizer, ref IQueryFieldExpression<TData> rootQueryFieldExpression,
            ...
            Tokenizer<TData> tokenizer, int positionBeforeToken, Func<string, TData> parseStringToDoc)
        {
            ...
            foreach (var functionCallParameterText in functionCallParametersText)
            {
                ...
                var resultingExpressionString = ParseEvaluationRecursive(parentTokenizer, ref rootQueryFieldExpression, functionCallParameterText, ref queryFields, parseStringToDoc);
                ...
                else if (IsNumericExpression(resultingExpressionString, parseStringToDoc, rootQueryFieldExpression.FunctionDependencies))
                {
        ...
        /// </summary>
        public static bool IsNumericExpression(string expressionText, Func<string, TData> parseStringToDoc, List<IQueryFieldExpressionFunction>? functionDependencies = null)
        {
            Tokenizer<TData> tokenizer = new(expressionText, [' ', '+'], parseStringToDoc);
            ...
            while (!tokenizer.IsExhausted())
            {
                ...
                if (token.StartsWith("$x_") && token.EndsWith('$'))
                {
                    ...
                    {
                        throw new KbParserException(tokenizer.GetCurrentLineNumber(), $"Function reference found without root expression: [{token}].");
                    }
        ...
        /// </summary>
        public static bool IsConstantExpression(string? expressionText, Func<string, TData> parseStringToDoc)
        {
