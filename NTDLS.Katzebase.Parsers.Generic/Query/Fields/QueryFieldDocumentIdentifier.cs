...
namespace NTDLS.Katzebase.Parsers.Query.Fields
{
    ...
    /// </summary>
    public class QueryFieldDocumentIdentifier<TData> : IQueryField<TData> where TData : IStringable
    {
        /// <summary>
        ...
        {
            var clone = new QueryFieldDocumentIdentifier<TData>(Value.ToString().EnsureNotNull(), ParseStringToDoc)
            {
        ...
        public Func<string, TData>? ParseStringToDoc = null;

        public QueryFieldDocumentIdentifier(string value, Func<string, TData> parseStringToDoc)
        {
            //Value = value.Trim().ParseToT<TData>(EngineCore<TData>.StrParse);
            ParseStringToDoc = parseStringToDoc;
            Value = ParseStringToDoc(value.Trim());
