    ...
    public class PolicyManager
    {
        ...
        internal KbQueryResult GrantRole(Transaction transaction, string schemaName, Guid roleId, SecurityPolicyPermission permission, bool isRecursive)
        {
            try
            {
                ...
                if (permission == SecurityPolicyPermission.All)
                {
                    ...
                    {
                        var newPolicy = new PhysicalPolicy
                        {
                        ...
                        _core.IO.PutPBuf(transaction, rdb, KbColumnFamilyName.Policy, new RdbKey(newPolicy.PolicyId), newPolicy);
                    }
                }
                else
                {
        ...
        internal List<AccountPolicyDescriptor> GetSchemaSecurityPolicies(Transaction transaction, string schemaName)
        {
            try
            {
                ...
                foreach (var policy in policyCatalog)
                {
