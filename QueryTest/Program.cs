    ...
    internal static class Program
    {
        ...
        static void Main()
        {
            try
            {
                ...
                var words = client.Query.Fetch<Word>("SELECT TOP 100 length((Text + @MyText) + Sha1('ooo')) as Len, Text, LanguageId, Id, SourceId, IsDirty FROM WordList:Word WHERE Text LIKE 'Fly%'", new { MyText = "Fly" });
